...
N_it = 100; % Anzahl der Iterationen

holo_size = 100;

input_phase = rand(holo_size,holo_size) * 2*pi - pi;
input_amp = ones(holo_size,holo_size)/1000000;
...
for i=1:N_it
    ...
    input_fft_amp(holo_size/2-gridSize/2:holo_size/2+gridSize/2-1, holo_size/2-gridSize/2:holo_size/2+gridSize/2-1) = abs(mode_target_distribution);
    input_fft_phase(holo_size/2-gridSize/2:holo_size/2+gridSize/2-1, holo_size/2-gridSize/2:holo_size/2+gridSize/2-1) = angle(mode_target_distribution);
...
target_fft = fftshift(fft2(input));

figure; imagesc(abs(mode_target_distribution))
figure; imagesc(angle(mode_target_distribution))

figure; imagesc(abs(target_fft))
figure; imagesc(angle(target_fft))
