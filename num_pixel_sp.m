...
bitDepthSLM = 8;            % Bit
superPixel=sp;               % Size of one superpixel
m_SLM=800;      % Y-axis
...
ref_background_Superpixel=superpixel_variabel(ref_background,2,bitDepthSLM,1, 2);
...
mode_target_distribution_superpixel = superpixel_variabel(mode_target_distribution,superPixel,bitDepthSLM,maximum, sp);
...
xo_pinhole=600;yo_pinhole=400;r_pinhole=20;
...
fidelity_sp(lauf) = max(fidelity_vals)
sp_nums(lauf) = sp
lauf = lauf+1;
end
figure;
plot(sp_nums, fidelity_sp); title('Fidelity in Abhänigigkeit von der Superpixelzahl');
xlabel('Superpixelzahl'); ylabel('Fidelity');
