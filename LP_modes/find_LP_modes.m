...
% ------------------------------------------------------------------------
function [solution, totalNumModes, map, map1] = find_LP_modes(coreRadius, nCore, nCladding, wavelength)
    ...
    while 1==1            
        ...
        for i = 1:length(fineURange)
            residual(i) = calculate_LP_mismatch(coreRadius, nCore, nCladding, wavelength, iOrder, fineURange(i));
            if i > 1
        ...
        for iSolution = 1:length(coarseSolutions)
            ...
            solution(end+1).u = fzero(@(u)calculate_LP_mismatch(coreRadius, nCore, nCladding, wavelength, iOrder, u),[minRange, maxRange]);
                        
            % Record the solution
            solution(end).order = iOrder;
            ...
            map(iOrder+1,iSolution)=1;
            map1(iOrder+1,iSolution)=2*iSolution+iOrder-1;
        end
        
        % If there are no solutions for this order then we are done
