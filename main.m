...
gridSize = 25;             % gridsize for modesolver; Wert am:06.04: 50
...
ref_background_Superpixel=superpixel_variabel(ref_background,2,bitDepthSLM,1,1);
...
mode_target_distribution_superpixel = superpixel_variabel(mode_target_distribution,superPixel,bitDepthSLM,maximum, 2);
...
%% Start modulation
%% ones statt optical beam
laser = ones(size(superpixel_mask, 1),size(superpixel_mask, 2));
...
modulated_beam=laser.*exp(1i*superpixel_mask);

% modulierter Laserstrahl propagiert durch Linse -> FFT
modulated_beam_fft=fftshift(fft2(modulated_beam));

% modulated_beam_fft(201,1) = 0;
% modulated_beam_fft(601,1) = 0;
% 
% modulated_beam_fft(201,800) = 0;
% modulated_beam_fft(601,800) = 0;
...
xo_pinhole=600;yo_pinhole=400;r_pinhole=40;
...
xo_pinhole_var=xo_pinhole-1:step_var:xo_pinhole+1;
yo_pinhole_var=yo_pinhole-1:step_var:yo_pinhole+1;
r_pinhole_var=r_pinhole-35:step_var:r_pinhole+35;
% r_pinhole_var=r_pinhole:step_var:r_pinhole+5;
...
%             fidelity_vals(index)=abs(g)^2;
            ...
            fidelity_vals(index) = our_calc_fidelity(mode_target_distribution_analyzis, modulated_beam_fft_reconstruct, area_analysis);

%               fidelity_vals(index) = complex_ssim(mode_target_distribution_analyzis, modulated_beam_fft_reconstruct, area_analysis);
...
[modulated_beam_fft_reconstruct_abs_cut_best,modulated_beam_fft_reconstruct_phase_cut_best]=cutout(abs(modulated_beam_fft_reconstruct_best),angle(modulated_beam_fft_reconstruct_best),area_analysis);

            zero_padding=size(mode_target_distribution_analyzis,1)-size(modulated_beam_fft_reconstruct_abs_cut_best,1);          
            modulated_beam_fft_reconstruct_abs_cut_best_padded=symmetric_zero_padding(modulated_beam_fft_reconstruct_abs_cut_best,zero_padding);
            modulated_beam_fft_reconstruct_phase_cut_best_padded=symmetric_zero_padding(modulated_beam_fft_reconstruct_phase_cut_best,zero_padding);

sig_size = num2str(gridSize*superPixel);
best = round(max(fidelity_vals),3);
save(strcat(root, sig_size, 'x', sig_size, 'sp-', num2str(superPixel), '_bit-' ,num2str(bitDepthSLM)), 'best');
...
subplot(3,2,3);
imagesc(abs(mode_target_distribution));title('desired amplitudedistribution');axis image
subplot(3,2,4);
imagesc(angle(mode_target_distribution));title('desired phasedistribution');axis image
subplot(3,2,5);
imagesc(abs(modulated_beam_fft_reconstruct_best));title('modulated amplitudedistribution');axis image
subplot(3,2,6);
imagesc(angle(modulated_beam_fft_reconstruct_best));title('modulated phasedistribution');axis image
...
% subplot(3,2,5);
% imagesc(modulated_beam_fft_reconstruct_abs_cut_best);title('Modulierte Amplitudenverteilung');axis image
% subplot(3,2,6);
% imagesc(modulated_beam_fft_reconstruct_phase_cut_best);title('Modulierte Phasenverteilung');axis image