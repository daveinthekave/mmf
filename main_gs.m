...
mode_target = 15;             % which mode should be generated by SLM?
mode_target_distribution = squeeze(modes(mode_target,:,:));
% [gs_mask, mask] = dcgs(optical_beam, mode_target_distribution, bitDepthSLM);
[gs_mask, mask] = dcgs(ones(desired_beam_size), mode_target_distribution, bitDepthSLM);
...
subplot(3, 2, 3);
imagesc(abs(modulated_input_fft .* mask));title('Amp moduliert nach fft'); axis image
subplot(3, 2, 4);
imagesc(angle(modulated_input_fft .* mask));title('Phase moduliert nach fft'); axis image
subplot(3, 2, 6);
