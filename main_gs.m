...
gridSize = round(desired_beam_size * sqrt(rel_area));             % gridsize for modesolver; Wert am:06.04: 50
...
gs_mask = dcgs(ones(desired_beam_size), mode_target_distribution, bitDepthSLM);

% moduliere beam
% modulated_input = abs(optical_beam) .* exp(1i*gs_mask); % abs richtig???
modulated_input = ones(desired_beam_size) .* exp(1i*gs_mask); % abs richtig???

modulated_input_fft = fftshift(fft2(modulated_input));
start = desired_beam_size/2 - gridSize/2;
stop = desired_beam_size/2 + gridSize/2 - 1;
modulated_input_fft_cut = modulated_input_fft(start:stop, start:stop);
...
subplot(3, 2, 3);
imagesc(abs(modulated_input_fft_cut));title('Amp moduliert nach fft'); axis image
subplot(3, 2, 4);
imagesc(angle(modulated_input_fft_cut));title('Phase moduliert nach fft'); axis image
subplot(3, 2, 6);
imagesc(gs_mask);title('Phase maske'); axis image
figure;
subplot(1, 2, 1);
imagesc(abs(modulated_input_fft));
subplot(1, 2, 2);
imagesc(angle(modulated_input_fft));
